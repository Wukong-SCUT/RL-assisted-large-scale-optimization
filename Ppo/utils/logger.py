...
def log_to_tb_train(tb_logger, agent, Reward, ratios, bl_val_detached, total_cost, grad_norms, reward, entropy, approx_kl_divergence,
               reinforce_loss, baseline_loss, log_likelihood, initial_cost, baseline, show_figs, mini_step):
    ...
    max_reward = torch.cat(reward).max()#torch.stack(reward, 0).max(0)[0].mean().item()

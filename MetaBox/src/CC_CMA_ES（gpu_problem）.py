...
shift=torch.from_numpy(np.random.uniform(lb, ub, size=D))
rotate=torch.eye(D)
...
#cma-es初始化
random_vectors = torch.from_numpy(np.random.uniform(lb, ub, size=(200, D)))
global_C = np.eye(D)
global_Xw = np.random.uniform(lb, ub, size=D)
...
best= random_vectors[min_index].numpy() #将best返还至numpy(因为后面的cma-es只接受numpy)
...
for _ in  tqdm(range(300),desc = "Overall Process"): # fes = 3e6:
    ...
    for i in range(m):
        ...
        with warnings.catch_warnings():
            ...
            sub_es = cma.CMAEvolutionStrategy(sub_centroid, sigma, {'popsize': sub_popsize,'bounds': [lb, ub]}) 
        ...
        while not sub_es.stop() and sub_cycle < sub_cycle_max:
            ...
            for ind in sub_population:
                sub_ind_to_global = torch.from_numpy(best.copy())

...
class Strategy(object): # 个体的策略
    ...
    def __init__(self, centroid, sigma, **kargs):
        ...
        self.centroid = numpy.array(centroid) # 个体的中心点

        self.dim = len(self.centroid) # 个体的维度
        self.sigma = sigma # 个体的标准差
        # 个体的历史最佳位置（pc）通常用于跟踪个体在搜索空间中的最佳位置，而全局历史最佳位置（ps）用于跟踪整个种群的最佳位置
        self.pc = numpy.zeros(self.dim) #数组表示了个体历史最佳位置
        self.ps = numpy.zeros(self.dim) #数组表示了全局历史最佳位置
        #计算PSO算法中的一个参数 chiN，通常用于调整粒子群的速度更新公式
        self.chiN = sqrt(self.dim) * (1 - 1. / (4. * self.dim)
                                      + 1. / (21. * self.dim ** 2)) 

        self.C = self.params.get("cmatrix", numpy.identity(self.dim)) # 个体的协方差矩阵
        self.diagD, self.B = numpy.linalg.eigh(self.C) # 个体的特征值和特征向量

        indx = numpy.argsort(self.diagD) # 对特征值进行排序
    ...
    def generate(self, ind_init):
        ...
        *ind_init* from the current strategy.
        ...
        """
        arz = numpy.random.standard_normal((self.lambda_, self.dim)) # 个体的随机向量
        arz = self.centroid + self.sigma * numpy.dot(arz, self.BD.T) # 个体的随机向量乘以特征向量乘以特征值
    ...
    def update(self, population):
        ...
        *population*.
        ...
        :param population: A list of individuals from which to update the
                           parameters.
        """
        population.sort(key=lambda ind: ind.fitness, reverse=True) # 对个体进行排序

        old_centroid = self.centroid # 个体的历史最佳位置
        self.centroid = numpy.dot(self.weights, population[0:self.mu]) # 个体的中心点
        ...
        self.ps = (1 - self.cs) * self.ps \
            ...
            * numpy.dot(self.B, (1. / self.diagD)
                        * numpy.dot(self.B.T, c_diff)) # 个体的全局历史最佳位置
